...
import { FlexBox, CustomText, WindowBox } from "../../component";
import { useRouter } from "next/navigation";

export const AppInfo = ({ isOpen, setClose }) => {
  useEffect(() => {
    console.log("##isOpens", isOpen);
  }, [isOpen]);
  return (
    <WindowBox
      windowVisible={isOpen}
      setWindowDelete={setClose}
      msg={
        <FlexBox direction="column" style={{ padding: 10 }}>
          <CustomText>
            안녕 여러분! 저는 이 앱의 개발자입니다. 지금부터 저의 애플리케이션에
            대한 소개를 할까 합니다. '이름 짓기'라는 주제로 앱을 만들게 된 건,
            다들 겪는 시간 낭비를 줄이기 위해서였습니다. 한 번 쯤은 게임
            닉네임을 뭘로 할지 고민하지 않으셨나요? 간지 작살나는 닉네임을 짓고
            싶은 마음은 굴뚝같지만 생각이 나지 않아 골치 아프셨을 겁니다.
          </CustomText>
